...
node {
    ...
    buildlib.withAppCiAsArtPublish() {
        commonlib.shell(
            script: """
                scl enable rh-python38 -- python3 release-tool.py -a ${params.ARCH} ${confirm_param} --kubeconfig ${KUBECONFIG} --context ocp/api-ci-l2s4-p1-openshiftapps-com:6443/system:serviceaccount:ocp:art-publish ${action} ${params.RELEASE_NAME}
                """,
        )
    }
